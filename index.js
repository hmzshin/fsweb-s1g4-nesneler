...
function MenuElemaniOlustur(value1, value2, value3) {
  const newObject = {};
  newObject.isim = value1;
  newObject.fiyat = value2;
  newObject.kategori = value3;
  return newObject;
}
...
console.log(MenuElemaniOlustur("Trileçe", 8, "Tatlılar"));
console.log(MenuElemaniOlustur("Ayran", 2, "İçecekler"));
/* Görev 2: 
...
console.log(`burger indirimli fiyatı: ${burger.indirim("öğretmen")}`);
console.log(`burger indirimli fiyatı: ${burger["indirim"]("öğretmen")}`);
...
*/
degerlendirmeler[degerlendirmeler.length - 1].degerlendirmeler =
...
function DegerlendirmeEkle(new_object, name, point, feedback) {
  new_object[new_object.length] = {};
  new_object[new_object.length - 1].isim = name;
  new_object[new_object.length - 1].puan = point;
  new_object[new_object.length - 1].geribildirim = feedback;
...
function SonDegerlendirmeyiAl(array) {
  return `${array[array.length - 1].isim} isimli kişi ${
    array[array.length - 1].puan
  } puan verdi ve şunları yazdı: ${array[array.length - 1].geribildirim}`;
}
